...
TEST(PropagatorTest, PropagatorWorks) {
    ...
    KeplerianElements const finish = propagate(start, 986e14, 5);
    std::cout << start.trueAnomaly << ' ' << finish.trueAnomaly << '\n';
    double const& meanMotion = std::sqrt(986e14) / std::pow(6378137, 1.5);
    double const& M1 = EccentricToMean(start.eccentricity, TrueToEccentric(start.eccentricity, start.trueAnomaly));
    double const& M2 = EccentricToMean(finish.eccentricity, TrueToEccentric(finish.eccentricity, finish.trueAnomaly));
