...
std::filesystem::path projectPath() {
    ...
    auto const pos = filePath.find("orbit_gravity_pressure.cpp");
...
int main() {
    ...
    std::string const DIR_PATH = FILE_PATH.substr(0, FILE_PATH.size() - 26);
    ...
    Time<Scale::TT> const endTime = Time<Scale::TT>::fromMJD(58877);
    ...
    Satellite<EarthGravity, SolarRadiationPressure> satelliteRHS{dutContainer, EOPcontainer, gravity, solarRadiationPressure, satelliteMass, gravityParameters, solarRadiationPressureParameters};
    
    auto solution = integrate<RK4Tableau, Satellite<EarthGravity, SolarRadiationPressure>>(initialState, beginTime, endTime, 50, satelliteRHS);

    std::ofstream fileRK4(DIR_PATH + "results/" + "gravity_pressure" + ".csv");
    fileRK4 << "t,x,y,z,r\n";
