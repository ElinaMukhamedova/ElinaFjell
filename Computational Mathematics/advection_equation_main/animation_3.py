...
with open('resources_for_animation/upwind_scheme/3_withoutTime.csv', newline = '') as file:
    reader = csv.reader(file, quoting=csv.QUOTE_NONNUMERIC)
    data = list(reader)

fig, ax = plt.subplots()
t = np.linspace(0, 18.18, 37)
...
ax.set(xlim=[0, 20], ylim=[-1.1, 1.1], xlabel='x', ylabel='u(x,t)', title='Courant number = 1.01')
...
anima = animation.FuncAnimation(fig=fig, func=update, frames=37, interval=505)
anima.save(project_directpry_path+'/results/advection_equation/AdvectionEquation_3.gif', writer='pillow')
plt.show()